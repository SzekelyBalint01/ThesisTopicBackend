...
public class LoginController {
    ...
    @PostMapping("/login")
    public ResponseEntity<Optional<UserDto>> userAuthentication(@RequestBody CredentialsDto credentialsDto) throws FileNotFoundException {

        Boolean authenticated = (login.authentication(credentialsDto));


        Optional<UserDto> user = login.getUserByEmail(credentialsDto);
